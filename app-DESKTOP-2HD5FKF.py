...
def delete_event(event_id):
    try:
        with get_db_connection() as conn:  # Use the existing connection manager
            c = conn.cursor()
...
def delete_event_callback(event_id):
    try:
        if delete_event(event_id):
            st.success(f"Event {event_id} deleted successfully.")
            time.sleep(0.5)  # Brief pause to ensure the success message is visible
            st.rerun()  # Force a rerun of the app to refresh the data
...
def main():
    try:
        ...
        st.sidebar.image("data2x.png", use_column_width=True)
        ...
        # Use session state to manage page selection
        if "page" not in st.session_state:
            st.session_state.page = "Data Entry"  # Initialize with a default page
        ...
        st.session_state.page = selection.split(" ", 1)[1]
